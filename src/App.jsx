...
const INITIAL_MARKETS = {
  games: [
    { id: "lsf1", type: "game", title: "Aruba Sea Serpents vs Humongous Melonheads", subtitle: "Lunar League 
      options: [{ id: "lsf1_ss", label: "Aruba Sea Serpents", odds: -225 }, { id: "lsf1_mel", label: "Humongous Melonheads", odds: +180 }] },
    { id: "lsf2", type: "game", title: "Raccoon City Outbreak vs Sopher McDophers", subtitle: "Lunar League 
      options: [{ id: "lsf2_out", label: "Raccoon City Outbreak", odds: -110 }, { id: "lsf2_mcd", label: "Sopher McDophers", odds: -111 }] },
    { id: "gsf1", type: "game", title: "Gas House Gorillas vs Sunnydale Slayers", subtitle: "Galactic League 
      options: [{ id: "gsf1_gor", label: "Gas House Gorillas", odds: -200 }, { id: "gsf1_sla", label: "Sunnydale Slayers", odds: +161 }] },
    { id: "gsf2", type: "game", title: "Ursa Major Grizzlies vs R'lyeh Ancients", subtitle: "Galactic League 
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, width: 200 }}>
        {keys.map((k, i) => (
          <button key={i} onClick={() => press(k)} style={{ height: 54, background: k === "" ? "transparent" : "#ffffff", border: k === "" ? "none" : "1px solid #e2e8f0", borderRadius: 12, color: k === "
            {k}
          </button>
        ))}
      </div>
...
export default function App() {
  ...
  async function handleAddMarket() {
    ...
    if (addType === "game") {
      ...
      if (addSpread) {
        ...
        if (!isNaN(sp) && !isNaN(spOA) && !isNaN(spOB)) {
          ...
          newMarketsToAdd.push({ id: spId, type: "game", title: `${addTitle.trim()} — Spread`, subtitle: (addSubtitle.trim() || "Custom Market") + " 
      ...
      if (addOU) {
        ...
        if (!isNaN(ou) && !isNaN(ovO) && !isNaN(unO)) {
          ...
          newMarketsToAdd.push({ id: ouId, type: "game", title: `${addTitle.trim()} — O/U ${ou}`, subtitle: (addSubtitle.trim() || "Custom Market") + " 
    ...
    notify(`${newMarketsToAdd.length} market${newMarketsToAdd.length > 1 ? "s" : ""} added! 
  }
        ...
        <div style={S.loginCard}>
          ...
          {loginStep === "name" && (
            <>
              ...
              <button style={{ ...S.btnPrimary, opacity: inputName.trim() ? 1 : 0.4 }} onClick={handleNameSubmit} disabled={!inputName.trim()}>Continue 
          ...
          {loginStep === "pin_confirm" && (
            <>
              ...
              <button style={S.btnGhost} onClick={() => { setLoginStep("pin_create"); setPinInput(""); setPinError(""); }}>
            </>
          )}
        </div>
      </div>
  ...
  if (screen === "admin") return (
    <div style={S.adminWrap}>
      <div style={S.adminHeader}>
        <button style={S.backBtn} onClick={() => { setAdminUnlocked(false); setAdminPin(""); setScreen("login"); }}>
      ...
      {!adminUnlocked ? (
        <div style={S.pinWrap}>
          ...
          <button style={S.btnPrimary} onClick={() => adminPin === ADMIN_PIN ? setAdminUnlocked(true) : notify("Wrong PIN", "error")}>Unlock 
        </div>
      ) : (
        <>
          <div style={S.adminTabRow}>
            {[["settle"," Settle"],["players"," Players"],["add"," Add"],["edit"," Edit"],["bets","
            ...
            {adminTab === "settle" && (
              <>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button style={{ ...S.settleBtn, flex: 1, textAlign: "center", background: "#fffbeb", borderColor: "#fde68a", color: "#d97706" }} onClick={pauseAllMarkets}>
                  <button style={{ ...S.settleBtn, flex: 1, textAlign: "center" }} onClick={unpauseAllMarkets}>
                </div>
                {/* Settle markets */}
                            ...
                            {market.options.map(opt => {
                              ...
                              if (isElim) {
                                return (
                                  <div key={opt.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 14px", background: "#fff1f2", border: "1px solid #fecaca", borderRadius: 8, opacity: 0.6 }}>
                                    <span style={{ fontSize: 13.0, color: "#ef4444", fontWeight: 600 }}>
                              ...
                              return (
                                <div key={opt.id} style={{ display: "flex", gap: 6 }}>
                                  <button style={{ ...S.settleBtn, flex: 1 }} onClick={() => settleMarket(market.id, opt.id)}>
                                  {market.type === "future" && (
                                    <button
...
↩ Unsettle
                            ...
                            <div
                              ...
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                <span style={{ fontSize: 11.0, color: "#94a3b8" }}>{expandedSettled === market.id ? "
                    ...
                    <div key={name}>
                      <div style={{ ...S.leaderRow, alignItems: "flex-start", paddingTop: 10, paddingBottom: 10 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                          ...
                          <button style={{ ...S.adjustBtn, fontSize: 11.0, padding: "3px 8px", color: "#ef4444", borderColor: "#fecaca", background: "#fff1f2" }} onClick={() => { if (window.confirm(`Delete ${name}?`)) deleteUser(name); }}>
                        </div>
                      </div>
                      {adjustingUser === name && (
...
── ADD TAB ── */}
            {adminTab === "add" && (
              <div style={S.adminSection}>
                ...
                <div style={S.formRow}><label style={S.formLabel}>SUBTITLE <span style={{ color: "#64748b" }}>(optional)</span></label><input style={S.input} placeholder="e.g. Lunar League 
                <div style={S.formRow}><label style={S.formLabel}>TYPE</label><div style={S.toggleRow}><button style={{ ...S.toggleBtn, ...(addType === "game" ? S.toggleBtnActive : {}) }} onClick={() => setAddType("game")}>Game</button><button style={{ ...S.toggleBtn, ...(addType === "future" ? S.toggleBtnActive : {}) }} onClick={() => setAddType("future")}>Future</button></div></div>
                <div style={S.formRow}><label style={S.formLabel}>TITLE</label><input style={S.input} placeholder="e.g. Aruba Sea Serpents vs Raccoon City Outbreak" value={addTitle} onChange={e => setAddTitle(e.target.value)} /></div>
                <div style={S.formRow}><label style={S.formLabel}>SUBTITLE <span style={{ color: "#64748b" }}>(optional)</span></label><input style={S.input} placeholder="e.g. Lunar League 
            ...
            {adminTab === "edit" && !editingMarket && (
              <>
                ...
                <div style={S.adminSection}>
                  ...
                  <textarea style={{ ...S.input, minHeight: 70, resize: "vertical", lineHeight: 1.5 }} placeholder="e.g. 
                ...
                <div style={S.adminSection}>
                  ...
                  {allMarkets.map(market => (
                    <div key={market.id} style={S.settleCard}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div><div style={{ fontSize: 14.0, fontWeight: 700, marginBottom: 2 }}>{market.title}</div><div style={{ fontSize: 11.0, color: market.status === "paused" ? "#f59e0b" : "#2e3a4e" }}>{market.status === "paused" ? "
              ...
              <div style={S.adminSection}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <div>
                    ...
                    {editingMarket.status === "paused" && <p style={{ margin: "4px 0 0", fontSize: 11.0, color: "#d97706" }}>
                  </div>
                ...
                <button style={{ ...S.btnPrimary, marginTop: 14, width: "100%" }} onClick={saveEdit}>Save Changes 
              </div>
            {/* 
── BETS TAB ── */}
            {adminTab === "bets" && (
              <div style={S.adminSection}>
                ...
                {[...bets].reverse().map(b => (
                  <div key={b.id} style={S.betRow}>
                    <div style={S.betRowTop}><span style={S.betRowUser}>{b.username}</span><span style={{ fontSize: 12.0, fontWeight: 700, color: b.status === "won" ? "#16a34a" : b.status === "lost" ? "#ef4444" : b.status === "voided" ? "#94a3b8" : "#d97706" }}>{b.status === "won" ? "
                    {b.betType === "parlay" ? (<><div style={{ fontSize: 11.0, color: "#3b82f6", marginBottom: 4, letterSpacing: 1 }}>PARLAY 
            ...
            {slipEntries.length > 0 && (
              <div style={S.slipFooter}>
                ...
                {slipMode === "parlay" && parlayEligible && (
                  <div style={{ marginBottom: 10 }}>
                    <div style={{ fontSize: 11.0, color: "#94a3b8", letterSpacing: 1, marginBottom: 6 }}>{slipEntries.length}-LEG PARLAY 
                ...
                <button style={S.placeBetBtn} onClick={placeBets}>{slipMode === "parlay" ? "Place Parlay 
              </div>
            )}
          </>
        )}
                ...
                return (
                  <div key={name} style={{ ...S.boardRow, borderBottom: !isLast ? "1px solid #1a1a24" : "none" }}>
                    <div style={S.boardLeft}>
                      ...
                      <span style={{ ...S.boardName, color: name === username ? "#6366f1" : "#d0d0d0" }}>{name}{name === username ? " 
                    </div>
                    <div style={S.boardRight}>
                      ...
                      <span style={{ fontSize: 12.0, color: diff >= 0 ? "#16a34a" : "#ef4444" }}>{diff >= 0 ? "
                    </div>
                  </div>
                );
              })}
                ...
                {inactivePlayers.map(({ name, u }, i) => (
                  <div key={name} style={{ ...S.boardRow, borderBottom: i < inactivePlayers.length - 1 ? "1px solid #1a1a24" : "none" }}>
                    <div style={S.boardLeft}>
                      ...
                      <span style={{ ...S.boardName, color: name === username ? "#6366f1" : "#555" }}>{name}{name === username ? " 
                    </div>
                    <div style={S.boardRight}>
            ...
            {[...userBets].reverse().map(b => (
              <div key={b.id} style={{ ...S.betCard, ...(b.status === "won" ? S.betCardWon : b.status === "lost" ? S.betCardLost : b.status === "voided" ? { opacity: 0.5 } : {}) }}>
                <div style={S.betCardTop}>
                  <span style={S.betMarket}>{b.betType === "parlay" ? <span style={{ color: "#3b82f6" }}>PARLAY 
                  <span style={{ fontSize: 12.0, fontWeight: 700, color: b.status === "won" ? "#16a34a" : b.status === "lost" ? "#ef4444" : b.status === "voided" ? "#94a3b8" : "#d97706" }}>{b.status === "won" ? "
                </div>
                {b.betType === "parlay" ? b.legs.map((l, i) => <div key={i} style={{ fontSize: 13.0, color: "#64748b", marginBottom: 3, paddingLeft: 4 }}><span style={{ color: "#6366f1", marginRight: 6 }}>{fmt(l.odds)}</span>{l.optionLabel}<span style={{ marginLeft: 6, fontSize: 11.0, color: l.status === "won" ? "#16a34a" : l.status === "lost" ? "#ef4444" : "#94a3b8" }}>{l.status === "won" ? "
